...
describe("service worker", () => {
  ...
  it("fetches a single release by tag and maps its assets", async () => {
    const state = await loadWorker("token");
    vi.mocked(fetch).mockResolvedValue(
      jsonResponse({
        tag_name: "v2.0.0",
  ...
  it("encodes the tag and works anonymously, tolerating a release with no assets", async () => {
    const state = await loadWorker();
    vi.mocked(fetch).mockResolvedValue(jsonResponse({ tag_name: "release/1.0" }));
